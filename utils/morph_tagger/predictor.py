...
class predict:
    ...
    def decode_l(self, feature, label):
        ...
        """
        # lstm_str = list()
        # temp = lstm_out.cpu().detach().numpy()
        # for s in temp:
        #     lstm_str.append(str(s))
        return '\n'.join(map(lambda t: t[0] + ' '+ self.r_l_map[t[1].item()] , zip(feature, label)))

        # return '\n'.join(map(lambda t: t[0] + ' '+ self.r_l_map[t[1].item()] + ' '+' '.join(lstm_str), zip(feature, label,lstm_out)))
    ...
    def output_batch(self, ner_model, documents, fout, file_no):
        ...
        for d_ind in tqdm( range(0, d_len), mininterval=1,desc=' - Process', leave=False, file=sys.stdout):
            ...
            for ind in range(0, f_len, self.batch_size):
                ...
                # labels,lstm_out = self.apply_model(ner_model, features[ind: eind], file_no)
